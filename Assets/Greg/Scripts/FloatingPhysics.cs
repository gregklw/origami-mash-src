...
namespace LudumDare_Greg
{
    ...
    public class FloatingPhysics : MonoBehaviour
    {
        ...
        public Transform waveObjT;

        //used components
        protected Rigidbody Rigidbody;
        protected TestWaves Waves;
        ...
        {
            //get components
            Waves = FindObjectOfType<TestWaves>();
            ...
            waveObjT = GameObject.FindGameObjectWithTag("Wave").transform;
        ...
        void FixedUpdate()
        {
            ...
            for (int i = 0; i < FloatPoints.Length; i++)
            {
                ...
                //if points are underwater then slowly float to top else position = surface
                WaterLinePoints[i].y = Waves.GetHeight(FloatPoints[i].position) + (waveObjT.position.y + GetComponent<MeshFilter>().mesh.bounds.size.y * 1.5f);
